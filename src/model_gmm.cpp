...
void GMM_random_root(Root &r) {
  ...
  // by producing a point with x1 + ... + x3 = 1 and scaling
  // by s, s is distributed according to f(x) = 3x^2 (cdf is F(x)=x^3).
  GMM_random_stochastic_vector3(r);

  // s = uniform_real(0.887904,0.928318);// New interval by Marta
 s = uniform_real(0.343,0.512);// New interval by Marta
  s = pow(s, (double)1 / 3);  
  ...
  r[3] = 1 - s;
  //New by Marta:
  while(r[0]<0.2 || r[1]<0.2 || r[2]<0.2 || r[0]>0.3 || r[1]>0.3 || r[2]>0.3){
    GMM_random_stochastic_vector3(r);
    //  s = uniform_real(0.887904,0.928318);// New interval by Marta
    s = uniform_real(0.343,0.512); // New interval by Marta
 s = pow(s, (double)1 / 3); 
