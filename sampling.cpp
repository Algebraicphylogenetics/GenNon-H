...
void random_parameters_length(Tree &T, Model &Mod, Parameters &Par){
  ...
  for(k=0; k < Par.nedges; k++) {
    ...
    //Mod.random_edge_length((T.edges[k]).br, Par.tm[k]);
   }
  Mod.random_root(Par.r);
}
...
void random_state(Tree &T, Model &Mod, Parameters &Par, State &st) {
  ...
  ///New by Marta:
  /* 
 std::vector<std::vector<double> > nodedistrib;
  std::vector<double> paux;
  for (long n=0; n<T.nnodes;n++){ 
    nodedistrib.push_back(Par.r);
  }
  //nodedistrib[T.nleaves]=Par.r;
  */  
...
/*		  for(long col=0;col<4;col++){
       paux.push_back(nodedistrib[T.edges[e].s][0]*Par.tm[e][0][col]+nodedistrib[T.edges[e].s][1]*Par.tm[e][1][col]+nodedistrib[T.edges[e].s][2]*Par.tm[e][2][col]+nodedistrib[T.edges[e].s][3]*Par.tm[e][3][col]);
   }
   nodedistrib[T.edges[e].t]=paux;
   paux.clear();
st.s[T.edges[e].t] = discrete(nodedistrib[T.edges[e].t]);
*/
      }
    }
  }
